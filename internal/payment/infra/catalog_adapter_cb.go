...
func (a *CatalogAdapterWithCB) PurchaseProduct(ctx context.Context, productID, customerID, orderID, instanceID, initialPassword, hostname, os string) (paymentApp.PurchasedProduct, error) {
	return circuitbreaker.Execute(a.cb, func() (paymentApp.PurchasedProduct, error) {
		return a.inner.PurchaseProduct(ctx, productID, customerID, orderID, instanceID, initialPassword, hostname, os)
	})
}
