...
- 当前 `/healthz`（`cmd/api/main.go:584`）只返回 `{"status":"ok","version":"..."}` ，仅表示进程存活，不检查数据库连通性或 gRPC 监听状态
- 建议拆分：
  - `GET /healthz/live` — 进程存活（维持现有逻辑）
  - `GET /healthz/ready` — 就绪（数据库 ping + gRPC 监听成功）
  - `GET /healthz/startup` — 启动完成（迁移执行完毕）
- K8s 的 `livenessProbe` 和 `readinessProbe` 需要指向不同端点

### 8.3 优雅关机配置化

- 当前优雅关机超时硬编码为 10s（`cmd/api/main.go:864`：`context.WithTimeout(..., 10*time.Second)`）
- 建议将 `shutdown_timeout` 作为配置项暴露，以便与 K8s `terminationGracePeriodSeconds` 对齐
