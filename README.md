...
- 高并发优先设计
  - 面向下单、支付、开通等高峰场景，内置分层限流、自适应同步/异步切换、热点缓存、熔断与延迟任务，重点不是单次请求“跑得快”，而是在流量上来时仍然保持系统可用，不轻易打穿数据库和宿主机。
- 部署门槛低
  - 基于 Go 构建，可直接编译为独立二进制运行；小规模场景可使用 `SQLite` 单文件部署，减少前期环境依赖和运维负担，适合快速上线和低成本试运营。
- 单机起步，集群扩展
  - 支持 `SQLite` 与 `PostgreSQL`，既能以轻量单机模式启动，也能在业务增长后平滑扩展到多节点和更完整的生产架构，不需要一开始就引入复杂基础设施。
- 业务链路完整
  - 从用户认证、商品目录、订单、支付、账单，到实例开通、资源池管理和宿主机 Agent 通信，后端核心链路已经打通，适合作为主机业务平台的基础底座。
